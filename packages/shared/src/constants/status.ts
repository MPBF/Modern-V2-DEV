...
export const RollStageLabels: Record<string, { ar: string; en: string; color: string }> = {
...
export const PriorityLabels: Record<string, { ar: string; en: string; color: string }> = {
...
export function getStatusLabel(
  ...
  const entry = map[key];
  return entry ? { label: entry[lang], color: entry.color } : { label: key, color: "#6b7280" };
}
